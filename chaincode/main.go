...
func (s *SmartContract) Invoke(APIstub shim.ChaincodeStubInterface) sc.Response {
	...
	if function == "queryCar" {
		...
		return s.createFruit(APIstub, args)
	} else if function == "queryFruit" {
		return s.queryFruit(APIstub, args)
	} else if function == "changeFruitName" {
		return s.changeFruitName(APIstub, args)
	} else if function == "changeFruitOwner" {
		return s.changeFruitOwner(APIstub, args)
...
func (s *SmartContract) createFruit(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
	...
	if len(args) != 4 {
		return shim.Error("Invalid no. of arguments")
	}

	var fruit = Fruit{Name: args[1], Quantity: args[2], Owner: args[3]}
...
func (s *SmartContract) changeFruitOwner(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {

	if len(args) != 2 {
		return shim.Error("Incorrect no. of arguments")
	}

	fruitAsBytes, _ := APIstub.GetState(args[0])
	fruit := Fruit{}

	json.Unmarshal(fruitAsBytes, &fruit)
	fruit.Owner = args[1]
...
func (s *SmartContract) changeFruitName(APIstub shim.ChaincodeStubInterface, args []string) sc.Response {
